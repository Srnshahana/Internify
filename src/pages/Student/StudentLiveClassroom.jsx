    ...
    try {
      ...
      if (data) {
        ...
        if (courseId && mentorId && studentId) {
          ...
          if (progData) {
            progressMap = new Map(progData.map(p => [String(p.session_id), p.is_completed]));
          }
        }
        ...
        setSessions(data.map(session => ({
          ...session,
          status: progressMap.get(String(session.id || session.sessionId)) ? 'completed' : 'upcoming'
        })))
      }
    } catch (err) {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      messagesToSend.push({
        ...
        file_url: null,
        type: 'text',
        tempId: Date.now().toString() + '-text'
      })

      // Optimistically update the UI 
      const optimisticMsgs = messagesToSend.map(msg => ({
        ...msg,
        id: msg.tempId, 
        from: 'learner', 
        time: getCurrentTime(),
      ...
      const dbMsgs = messagesToSend.map(({ tempId: _temp, ...dbMsg }) => dbMsg)
      ...
      if (channelRef.current) {
        uiMsgs.forEach(msg => {
          channelRef.current.send({
            type: 'broadcast',
            event: 'chat-message',
            payload: { ...msg, from: userRole === 'mentor' ? 'mentor' : 'learner' } 
          })
        })
      }
  ...
  const handleImageSelected = async (e) => {
    ...
    try {
      ...
      for (const file of files) {
        const fileExt = file.name.split('.').pop()
        let finalType = 'image'
        if (file.type && file.type.startsWith('video/')) finalType = 'video'
        ...
        const { error: attachmentError } = await supabase
          ...
          .insert({
            ...
            file_name: file.name,
            file_url: publicUrl,
            description: `${finalType === 'image' ? 'Image' : 'Video'} Attachment`,
            created_at: new Date().toISOString()
          })
