...
function PendingWork({ onBack }) {
  ...
  const handleApprove = async (submissionId) => {
    try {
      ...
      showModal('Success', 'Work approved successfully!', 'success')
      setSelectedWorkId(null)
      fetchPendingWork()
    } catch (err) {
      console.error('Error approving work:', err)
      showModal('Error', 'Failed to approve: ' + err.message, 'error')
    }
  }

  const handleReject = async (submissionId) => {
    if (!feedback) {
      showModal('Validation Error', 'Please enter feedback before rejecting.', 'error')
      return
    }
    ...
    try {
      ...
      if (error) throw error
      showModal('Success', 'Work rejected with feedback.', 'success')
      setSelectedWorkId(null)
      setFeedback('')
      fetchPendingWork()
    } catch (err) {
      console.error('Error rejecting work:', err)
      showModal('Error', 'Failed to reject: ' + err.message, 'error')
    }
  }
  ...
  const renderContent = () => {
    if (loading) {
      return <div style={{ padding: '100px', textAlign: 'center' }}>Loading pending work...</div>
    }
